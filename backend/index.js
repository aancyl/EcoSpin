...
app.post("/addproduct", async (req, res) => {
  const newProduct = new Product({
    ...
    imageData: req.body.imageData, // base64
  });
...
app.post('/newsletter', async (req, res) => {
    try {
        ...
        res.status(201).json({ success: true, message: "User registered for newsletter successfully." });
...
app.post('/adminsignup', async (req, res) => {
    try {
        ...
        if (check) {
            return res.status(400).json({ success: false, error: "An admin with the same name already exists" });
        }
...
app.post("/upload", upload.single("product"), async (req, res) => {
  ...
  res.status(200).json({ success: 1, imageData });
});
...
app.put('/markcompleted/:orderId', async (req, res) => {
    try {
        ...
        const updatedOrder = await Order.findByIdAndUpdate(orderId, { completed }, { new: true });
        ...
        res.status(500).json({ success: false, error: "Error marking order as completed" });
    }
});
